...
export default function LoggedUser({user, handleLogOut}) {
  const [menuActive, toggleMenu] = useState(false)
    return (
    <Fragment>
    <div style={{display:'flex', justifyContent: 'flex-end', alignContent: 'center'}} className='user'>
       <Link to='/upload-game/'> <h4 title='upload game'>🔼</h4> </Link>
        <Link to={`/favorites/${user.id}`}><h4 title='View Favorites'>❤️</h4></Link>
        <img title='settings' onClick={()=>toggleMenu((pos)=>(!pos))} src={user.pfp} style= {{cursor:'pointer',maxWidth:'50px', height:'50px', borderRadius:'50%', objectFit:'cover'}}></img>
    </div>
